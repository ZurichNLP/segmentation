...
- **Sequence Encoder beats Autoregressive** on both, and trains far faster (~14 h vs ~1 day).
...
- **The corpora bias segmentation in opposite directions**: BOBSL over-segments (`%` 2.50 / 1.74), YouTube-ASL under-segments (`%` 0.70 / 0.55). They attribute this to differing annotation granularity between the two.
...
- **A warning for that pretraining stage.** Subtitle-derived boundaries are coarser than phrase annotation — YouTube-ASL gives `%` 0.70 here. Pretraining on them risks biasing toward the under-segmentation our phrase level already shows, so the bias should be measured rather than assumed away.
